...
int execute(t_cub *cub)
{
    ...
    set_player_dir(cub);
    init_textures(cub);
    mlx_hook(cub->mlx.win, KEY_PRESS, KEY_PRESS_MASK, key_hook, cub);
    mlx_hook(cub->mlx.win, MOUSE_MOVE, MOUSE_MASK, mouse_move, cub);
    mlx_hook(cub->mlx.win, CLOSE, CLOSE_MASK, close_win, cub);
