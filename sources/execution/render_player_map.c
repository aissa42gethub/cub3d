...
/*   Updated: 2025/10/02 17:14:33 by aaissa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	render_tiles(t_cub *cub, int px, int py, int tile_size)
{
	t_tile	tile;
	int		j;
	int		i;
	...
	while (i < MINIMAP_SIZE / 2)
	{
		...
		while (j < MINIMAP_SIZE / 2)
		{
			tile.mx = px + j;
			tile.my = py + i;
			tile.psx = MINIMAP_OFFSET + (j + MINIMAP_SIZE / 2) * tile_size;
			tile.psy = MINIMAP_OFFSET + (i + MINIMAP_SIZE / 2) * tile_size;
			which_tile(cub, &tile);
			j++;
		}
		i++;
